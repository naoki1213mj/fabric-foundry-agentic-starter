...
def track_event_if_configured(event_name: str, event_data: dict):
    ...
    if instrumentation_key:
        ...
        logging.warning(
            "Skipping track_event for %s as Application Insights is not configured", event_name
        )
...
class CosmosConversationClient:
    ...
    async def create_conversation(self, user_id, conversation_id=str(uuid.uuid4()), title=""):
...
async def add_conversation(user_id: str, request_json: dict):
    ...
    try:
        ...
        if messages and messages[-1]["role"] == "user":
            created_message = await cosmos_conversation_client.create_message(
                str(uuid.uuid4()), conversation_id, user_id, messages[-1]
            )
...
async def update_conversation(user_id: str, request_json: dict):
    ...
    if len(messages) > 0 and messages[-1]["role"] == "assistant":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No assistant message found"
        )
    await cosmos_conversation_client.cosmosdb_client.close()
    return {
        "id": conversation["id"],
...
async def update_message_feedback(
    user_id: str, message_id: str, message_feedback: str
) -> dict | None:
    """Update feedback for a specific message."""
    try:
        logger.info(f"Updating feedback for message_id: {message_id} by user: {user_id}")
        cosmos_conversation_client = init_cosmosdb_client()
        ...
        logger.exception(f"Error updating message feedback for message_id: {message_id}")
        raise
...
async def delete_conversation(user_id: str, conversation_id: str) -> bool:
    ...
    try:
        ...
        if conversation["userId"] != user_id:
            logger.warning(f"User {user_id} does not have permission to delete {conversation_id}.")
            return False
...
async def get_conversations(user_id: str, offset: int, limit: int | None):
    """
...
async def get_conversation_messages(user_id: str, conversation_id: str):
    ...
    try:
        ...
        logger.exception(f"Error retrieving conversation {conversation_id} for user {user_id}")
        return None
...
async def clear_messages(user_id: str, conversation_id: str) -> bool:
    ...
    try:
        ...
        logger.info(f"Successfully cleared messages in conversation {conversation_id}.")
        return True

    except Exception as e:
        logger.exception(f"Error clearing messages for conversation {conversation_id}: {e}")
        return False
...
async def update_message_feedback_route(request: Request):
    ...
    try:
        ...
        if not message_id:
            track_event_if_configured(
                "MessageFeedbackValidationError",
                {"error": "message_id is missing", "user_id": user_id},
            )
        ...
        if not message_feedback:
            track_event_if_configured(
                "MessageFeedbackValidationError",
                {"error": "message_feedback is missing", "user_id": user_id},
            )
        ...
        if updated_message:
            track_event_if_configured(
                ...
                {"user_id": user_id, "message_id": message_id, "feedback": message_feedback},
            )
            return JSONResponse(
                content={
            ...
            track_event_if_configured(
                "MessageFeedbackNotFound", {"user_id": user_id, "message_id": message_id}
            )
            raise HTTPException(
                status_code=404,
...
async def delete_conversation_route(request: Request):
    ...
    try:
        ...
        if deleted:
            track_event_if_configured(
                "ConversationDeleted", {"user_id": user_id, "conversation_id": conversation_id}
            )
            return JSONResponse(
                content={
...
async def list_conversations(
    request: Request, offset: int = Query(0, alias="offset"), limit: int = Query(25, alias="limit")
):
    ...
    try:
        ...
        if not isinstance(conversations, list):
            track_event_if_configured(
                "ListConversationsNotFound", {"user_id": user_id, "offset": offset, "limit": limit}
            )
            return JSONResponse(
                content={"error": f"No conversations for {user_id} were found"}, status_code=404
            )
        track_event_if_configured(
            "ConversationsListed",
            {
...
async def get_conversation_messages_route(request: Request):
    ...
    try:
        ...
        if not conversationMessages:
            track_event_if_configured(
                "ReadConversationNotFound", {"user_id": user_id, "conversation_id": conversation_id}
            )
            raise HTTPException(
                status_code=404,
        ...
        return JSONResponse(
            content={"conversation_id": conversation_id, "messages": conversationMessages},
            status_code=200,
        )
...
async def delete_all_conversations(request: Request):
    ...
    try:
        ...
        track_event_if_configured(
            "AllConversationsDeleted", {"user_id": user_id, "deleted_count": len(conversations)}
        )
...
async def clear_messages_route(request: Request):
    ...
    try:
        ...
        if not success:
            track_event_if_configured(
                "ClearMessagesFailed", {"user_id": user_id, "conversation_id": conversation_id}
            )
            raise HTTPException(
                status_code=404, detail="Failed to clear messages or conversation not found"
            )
        track_event_if_configured(
            "MessagesCleared", {"user_id": user_id, "conversation_id": conversation_id}
        )
...
async def ensure_cosmos_route():
    ...
    try:
        ...
        if not success:
            track_event_if_configured(
                "CosmosDBEnsureFailed", {"error": err or "Unknown error occurred"}
            )
            return JSONResponse(content={"error": err or "Unknown error occurred"}, status_code=422)
        track_event_if_configured(
            "CosmosDBEnsureSuccess", {"status": "CosmosDB is configured and working"}
        )

        return JSONResponse(
            content={"message": "CosmosDB is configured and working"}, status_code=200
        )
    except Exception as e:
        ...
        if "Invalid credentials" in cosmos_exception:
            ...
            return JSONResponse(
                content={"error": "Invalid CosmosDB configuration"}, status_code=422
            )
        else:
            return JSONResponse(
                content={"error": "CosmosDB is not configured or not working"}, status_code=500
            )
