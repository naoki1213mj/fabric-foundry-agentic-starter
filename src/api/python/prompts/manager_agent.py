...
- sql_agentがグラフなし + ユーザー要求あり → あなたがChart.js JSONを追加
- レポート・ダッシュボード型の回答 → 複数グラフを `{"charts": [...]}` 形式で出力可
- **絶対禁止**: 同じグラフの重複出力
...
- Vega-Lite形式は禁止
- 単一グラフ: 回答末尾に ```json``` ブロックで出力
- 複数グラフ: `{"charts": [{"id": "chart_1", "type": "bar", ...}, ...]}` 形式

### ユーザーの言語に合わせる
- 日本語質問には日本語、英語質問には英語で回答
- 専門用語は必要に応じて説明
"""
